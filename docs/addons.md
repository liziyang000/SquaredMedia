...
| `douban` | 匹配、同步和校准豆瓣评分；提供后台任务、人工核查与全库体检 | `__PREFIX__douban_config`、`__PREFIX__douban_vod_meta`、`__PREFIX__douban_task`、`__PREFIX__douban_log`、`__PREFIX__douban_review_candidate`、`__PREFIX__douban_scan`、`__PREFIX__douban_scan_issue` | 无 | 已纳入打包、发布校验和 SSH 部署 |
...
4. 管理员可输入单个 MacCMS 视频 ID 立即获取豆瓣数据：已有豆瓣 ID 时直接同步，没有 ID 时先匹配；唯一高置信候选自动绑定，歧义候选可复用站点现有 AI 搜索配置做一次候选集内复核，然后继续同步或交给人工确认。
5. 失败任务按次数延后，达到最大次数后进入 `FAILED`；管理员可以批量重新入队，也可以手动同步、锁定 ID、忽略影片或校准评分。
6. 全库体检按视频 ID 游标分批读取本地数据，只把问题写入独立报告表；可暂停、续跑、筛选和导出，不访问豆瓣，也不修改视频表。

### 结构与入口

- `Douban.php`：插件主类，无运行时钩子。
- `application/admin/controller/Douban.php`：唯一的路由控制器，正式后台菜单入口为 `admin/douban/index`。
- `backend/DoubanController.php`：后台业务控制器实现，提供配置、单片获取、到期/定向任务生成、任务明细、Worker、失败重试、同步、校准、全库体检、ID 锁定和忽略动作；它不位于插件公开 `controller/` 目录。
- `service/DoubanData.php`：配置、任务、匹配、同步、校准、全库体检、失败退避和日志编排。
- `service/DoubanGateway.php`、`service/DoubanMatcher.php`：豆瓣数据请求/归一化与候选匹配排序。
- `view/index/index.html`：后台管理页面。
- `config.php`、`install.sql`：数据源、分类排除、匹配阈值、请求限速、最大尝试次数和七张插件表。
...
- 生成任务不会触发全表评分校准；到期入队把“尚无插件元数据”和“已有元数据且已到期”拆成两条查询，后者从 `douban_next_sync_at` 索引开始读取。定向入队只扫描确认后的分类和筛选范围。两者都一次预取已有活动任务、批量写入元数据和任务，避免按影片重复查询和插入。
- Worker 在同一批次复用一份配置，由 `worker_limit` 限制执行规模；后台把手动执行拆成每批最多 3 条并逐批反馈结果。所有 PHP Worker 通过配置表行锁预留请求时间槽，实现跨 Worker 共享限流。数据库限流状态不可用时才回退到当前进程内节流。插件本身不注册定时钩子，页面中的批量设置不会自行启动服务器计划任务。
...
- 全库体检固定使用 `vod_id > cursor AND vod_id <= high_water_vod_id` 的索引游标，每批最多 200 条，不使用随数据量退化的 OFFSET。浏览器在批次间留出间隔并实时更新进度；关闭页面不会继续占用 PHP Worker，任务游标保留在报告表中。
...
- 后台忽略影片时，会立即把该影片尚未执行的任务标记为 `SKIP`；Worker 在执行已领取任务前也会复查忽略期限，避免继续发起自动豆瓣请求。管理员手动单片同步仍作为显式操作保留。
- “待执行任务明细”默认显示前 50 条 `PENDING` 任务，并可切换查看运行中、失败、成功、跳过或全部状态；列表展示视频、分类、任务类型、尝试次数、计划执行时间和最近错误。

### 指定单片获取

- 后台默认只展示“查找视频 → 选择并匹配”主流程；批量任务设置、校准和日志统一收进默认折叠的“高级工具”，避免单片操作被维护入口干扰。
- 核查列表支持按影片名称或本地视频 ID、精确分类、精确年份和豆瓣状态组合查询本地 `vod` 数据；纯数字按 `vod_id` 精确查询，结果分页保留全部筛选参数。分类名称随结果一起展示，查询本身不访问豆瓣。
- 无豆瓣 ID 的查询结果会显示“搜索豆瓣ID”，点击后才进入单片匹配流程；已有 ID 的结果继续显示同步操作。
- 耗时请求会立即在页面顶部显示当前操作、真实等待秒数和完成结果；失败、候选待核查与成功使用不同状态，不再依赖阻塞式 `alert`。单片同步成功后会保留“本次同步数据”，逐字段展示同步前值和同步后值；超过 MacCMS 字段上限的数据会保留本地值并显示警告，不会阻断其他字段同步。管理员确认后再刷新列表，其他需要刷新的操作会在刷新后恢复最后一条结果提示。
- 手动同步、Pending Worker 和匹配到豆瓣 ID 后的继续同步都固定排除 `vod_pic`；即使数据源返回海报地址，也会保留 MacCMS 中的现有图片。
- 对旧版本已经同步的异常图片，只有当前图片仍与最近一次图片同步结果一致时，影片“更多操作”才显示“回退上次图片”。服务端直接定位最近一次包含 `vod_pic` 变更的 `AUTO_SYNC` 日志，不受后续非图片同步日志数量影响；当前图片后来已被修改或旧图片为空时会拒绝回退。该操作不会回退评分、简介等字段。
- “指定视频获取豆瓣数据”使用 MacCMS `vod_id` 精确定位影片，并在当前后台请求中立即执行，不需要先选择分类或运行 Pending Worker。
- 已有有效豆瓣 ID 时直接获取详情并同步；没有 ID 时复用候选搜索和标题/年份评分，唯一高置信候选会自动绑定后继续同步。
- 只有管理员手动单片搜索遇到歧义时才尝试 AI 复核；Pending Worker、定向任务和全库体检都不会调用 AI，因此不会把模型延迟或费用扩散到后台批处理。
- AI 复核直接读取站点服务端的 `maccms.ai_search` 配置并复用已有 `AiProvider`，插件不保存第二份 API Key，也不会把密钥发送到浏览器。站点 AI 搜索未启用、配置不完整、请求失败或返回格式异常时，流程自动回退到原候选人工核查。
- 模型只能从本次豆瓣搜索返回的候选 ID 中选择，候选集外 ID 会被拒绝。只有 AI 置信度至少 90、原有标题/年份匹配分至少 85 且无冲突时才自动绑定；否则只把 AI 推荐及置信度显示在候选列表中，由管理员确认。
- 候选存在歧义且未通过上述 AI 安全门时不会自动写入豆瓣 ID，而是保存候选并进入待核查列表；影片不存在、数据源失败或响应不合法时返回明确错误。
...
- 同一时刻只允许一个 `RUNNING` 或 `PAUSED` 任务。开始时记录视频总数和最高 `vod_id`，后续新增视频不会混入本次结果；暂停和失败恢复都从已保存游标继续。
- 每批只读取体检规则需要的视频字段，并一次性读取本批插件元数据。规则覆盖豆瓣 ID 缺失/冲突、评分缺失/不一致、年份缺失、地区或语言过长、同步失败、待人工核查和未启用状态。
- `douban_scan` 与 `douban_scan_issue` 均为独立 InnoDB 表；批次问题和游标在同一事务提交。体检流程不调用 `DoubanGateway`，不会更新 MyISAM `vod` 表，也不会自动生成同步任务。
- 页面逐批显示扫描数、问题数和游标，可暂停/恢复；报告支持按问题类型、视频名称或视频 ID 筛选，并由已登录管理员按问题类型流式导出 CSV。

### 安全边界

- 管理页面、报告导出和所有写操作通过 `model('Admin')->checkLogin()` 检查 MacCMS 后台登录态；写操作额外要求 POST 和同源页面才能发送的 `X-Requested-With: XMLHttpRequest` 请求头。
- `info.ini` 不提供前台链接，发布包不包含前台模块或公开插件控制器；管理功能只能经随机后台入口的 `admin/douban/*` 路由访问。
- 当前动作没有独立 CSRF Token；插件级 AJAX 来源校验可阻止普通跨站表单提交，但部署方仍应在所有自定义后台页面接入 Token 后启用站点全局 CSRF。
...
- `tests/douban-data.test.php`：覆盖 ID、评分、有限重试、限流时间槽、候选展示数据、定向筛选及全库体检规则。
- `tests/douban-worker.test.php`：通过内存表覆盖任务重试/租约/限流，以及体检游标、并发拒绝、暂停恢复、只读视频表和禁止外部豆瓣请求。
- `tests/douban-gateway.test.php`：覆盖豆瓣详情与候选数据归一化、评分边界和本地统计字段保护。
- `tests/douban-matcher.test.php`：覆盖候选匹配评分与阈值。
- `tests/douban-ai-reviewer.test.php`：覆盖站点 AI 配置复用、候选 ID 白名单和关闭状态零调用；Worker 测试同时约束 AI 只进入手动歧义匹配。
