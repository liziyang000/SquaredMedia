...
| --- | --- | --- | --- | --- |
| `pingfangdevice` | 管理会员设备会话；为主题提供动态筛选、按集线路检测和健康线路排序 | `__PREFIX__pingfang_device_session` | `app_begin` | 已纳入打包、发布校验和 SSH 部署 |
...
`VodFilterOptions` 和 `VodSourceQuality` 是同一插件内的附加视频能力。前者按已启用视频、栏目范围及当前筛选条件，返回地区、年份、语言选项和数量，并缓存 120 秒。后者接收已启用视频的 `vod_id` 和指定集数 `nid`，从服务端解析各播放组的同序号剧集；直链使用不同字节区间，HLS 最多使用 3 个不同媒体分片，至少取得 2 个有效样本后才返回中位速度。HLS 主清单还会按声明分辨率选择 Variant，最高 Variant 失败时最多再尝试 2 个较低 Variant，并缓存 60 秒。这两项能力不管理设备，但当前主题复用了同一个前台兼容控制器入口。
...
- `controller/DeviceActions.php`：两个控制器共用的登录、退出、撤销、筛选和线路检测动作。
...
- 当前主题中的登录表单、用户菜单、设备页、视频筛选和详情页线路检测端点与这些兼容路由保持一致。

### 测试定位

- `tests/device-session.test.php`：用内存数据库/模型替身覆盖原生登录接管、Token 重绑、撤销与过期、Cookie 名、设备上限、事务回滚和展示数据脱敏。
- `tests/device-controller.test.php`：通过共享 `DeviceActions` 覆盖兼容入口的登录参数归一化、POST/Ajax 约束、线路检测参数转发、设备注册失败时回滚原生登录，以及退出时清理设备 Token。
