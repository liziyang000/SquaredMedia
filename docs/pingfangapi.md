...
地区、年份、语言有类型/父类型/全局扩展配置时，以配置作为允许范围，并与当前
可用数据取交集；地区、语言保留配置顺序，年份按新到旧排序。没有交集时返回空数组，
不会回退到无数据的配置值。结果按权限、其他筛选条件及配置内容缓存，修改配置会
使用新的缓存键。地区和语言使用字段等值匹配，逗号组合值不会拆成无法匹配的单项；
剧情仍按标签拆分。无法作为合法查询参数的空值、超长值或带换行/制表符的值不会展示。
...
写入前会执行影片详情权限；影片不存在返回 404，无权访问返回 403。新增收藏、刷新
收藏时间或取消收藏的数据库操作返回失败时，接口返回 500，不会返回伪成功状态。
更新或删除返回 0 个受影响行仍属于幂等成功；取消收藏只匹配当前用户和当前影片的
收藏记录，不影响其他账号或观看历史。
...
排序，因此 `total` 是可展示影片数，不是原始 Ulog 行数。服务端以每批 100 条、
按 `ulog_time desc,ulog_id desc` 的内部游标扫描候选 Ulog，不再使用不断增大的
`OFFSET`；同一时间的记录以 ID 为边界，避免漏过批次末尾。已找到有效剧集的影片
会立即释放完整播放字段和解析列表，后续同片记录也不会重复加载它们。确定最终
当前页后，才为该页影片补查并聚合全部 `recordIds`。由于剧集有效性依赖 PHP 解析
`vod_play_url`，准确页码模式仍需遍历候选记录，不是只读取当前数据库页，也未增加
索引或修改表结构。真实数据库耗时仍需在部署前通过 staging 测量。
...
| 地区/年份/语言 | `summary_cache_seconds`，默认 1800 秒      | 分类 + 用户组权限 + 其他筛选条件 + 扩展配置内容                   |
...
- 地区、年份和语言选项将分类或全局扩展配置与当前权限、筛选条件下的可用数据取交集，并缓存结果；冷缓存仍需聚合查询。
- 分类数量和剧情选项由调用方显式按需请求。
- 组合筛选仍返回精确总数，并缓存计数结果。
- 目录精确总数、稳定分页、分类统计和筛选聚合不再强制 `PRIMARY` 或单列排序索引，允许数据库选择覆盖索引；状态、回收、权限谓词和 `vod_id desc` 排序次序不变。
- 精确计数和筛选聚合会在当前请求内检查一次 `idx_pfapi_catalog`；只有字段顺序完整、无前缀截断且可见的 BTREE 索引才受控选用，避免优化器误选不覆盖筛选字段的最新排序索引。索引不存在或不兼容时不强制使用；关键词、字母、可播放条件和空分类结果保留原优化器计划。
- 去掉索引提示不等于已经解决大表性能问题。缺少覆盖索引时，冷缓存查询仍可能超时；应先验证真实 SQL 的计划和耗时，再发布。
- 搜索使用前缀匹配，且不强制排序索引以避免错误的执行计划假设。

索引策略必须结合当前生产表的 `SHOW INDEX`、`EXPLAIN` 和只读计时验证；迁移数据库
或改变表引擎后需重新验证。复杂搜索仍可能较慢。大型 MyISAM 目录的候选索引及独立
审批步骤见[部署指南 9.9](pingfangapi-deployment-guide.md#99-目录冷缓存请求超过部署验收上限)。
索引变更可能锁表，不由 API 安装或部署脚本自动执行。
...
远端；它会检查文件/hook、PHP CLI 扩展、数据库连接/必要字段、部署用户目录权限和
可用空间，打印实际目标、`backend`/`api` 范围和判定原因。确认目标正确且当前
MacCMS 数据库备份已经完成后，再执行第二条命令并按提示输入 `deploy`。

部署入口会自动读取 `scripts/deploy-ping2.env`，不要求 `node_modules`，也不执行
`npm ci`；确认后由 `scripts/deploy-theme.sh` 运行 `test:backend`、组包和归档校验。
它不发布主题、Next 或联机游戏服务。已经完成审批、目标复核和数据库备份的非交互
任务可执行：
...
缺设备表或已有表仅缺 `login_check_hash` 时也会选择 `backend`；Ulog 进度列或
已有设备表的其他必需列缺失则在确认前停止，要求单独迁移，`--backend` 不能绕过。
入口通过 SSH 标准输入和底层通过暂存归档执行同一 `DeploymentCheck.php`，只读
检查不创建文件或执行 DDL。目录检查针对 CLI 用户，不替代 PHP-FPM 权限验收；
可用空间报告也不能代替按实际备份大小预留容量。
...
| `tests/pingfang-api-account-history.test.php` | 1 万条同时间记录、跨批次无效剧集、精确分页和大型播放字段释放 | 使用查询替身，不证明真实数据库耗时 |
| `tests/pingfang-api-facets.test.php` | 配置与数据交集、交叉条件、权限、组合值和缓存更新 | 不执行真实 MySQL 查询计划 |
| `tests/pingfang-api-deployment-check.test.php` | 只读字段诊断、受支持的迁移分类、目录与凭证脱敏；由部署命令测试调用 | 不连接真实服务器和数据库 |
