...
为 `pingfangvideo` 主题增加一套自用前端播放器，在 MacCMS 播放页中优先接管可直连播放的 `.m3u8` 和 `.mp4` 资源，提供统一控制栏、倍速、音量、全屏、错误回退和播放进度记忆。
...
- 模板结构文档定义主题静态资源目录包含 `js`、`css`、`images`、`html`，播放器脚本应放在 `template/pingfangvideo/js/` 并通过 `{$maccms.path_tpl}` 加载。

## 方案选择

选择“混合接管播放器”方案。

播放器脚本在 MacCMS 原播放器输出之后运行，读取 `window.player_data.url` 或本地预览页面中的 `<video>` 直链。只有在地址明确是 `.m3u8` 或 `.mp4` 时才创建 PingFang 播放器 UI；否则不改动页面。
...
- `template/pingfangvideo/js/pingfang-player.js`：播放器接管、UI、事件、进度记忆和回退逻辑。
- `template/pingfangvideo/js/hls.min.js`：本地 HLS 播放依赖。

修改文件：

- `template/pingfangvideo/html/vod/play.html`：当前保留 MacCMS 原播放器输出，不加载本地 HLS 和自用播放器脚本。
- `template/pingfangvideo/html/vod/player.html`：当前保留 MacCMS 原试看/收费播放器输出，不加载本地 HLS 和自用播放器脚本。
- `template/pingfangvideo/css/style.css`：新增 `.pf-player` 样式和响应式控制栏。
