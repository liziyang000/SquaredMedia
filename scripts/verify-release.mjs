...
assert.match(doubanAddonService, /public static function fetchVod/);
...
assert.match(doubanAddonService, /public static function auditDashboard/);
assert.match(doubanAddonService, /public static function startAudit/);
assert.match(doubanAddonService, /public static function runAuditBatch/);
assert.match(doubanAddonService, /public static function pauseAudit/);
assert.match(doubanAddonService, /public static function resumeAudit/);
assert.match(doubanAddonService, /const AUDIT_START_LOCK_KEY = 'audit_start_lock'/);
assert.match(doubanAddonService, /where\('config_key', self::AUDIT_START_LOCK_KEY\)\s*->lock\(true\)/);
assert.match(doubanAddonService, /where\('vod_id', '>', \$cursor\)/);
assert.doesNotMatch(doubanAddonService, /OFFSET\s+/i);
...
assert.match(doubanAddonService, /private static function syncChangesForView/);
assert.match(doubanAddonService, /'changes'\s*=>\s*self::syncChangesForView\(\$oldValues, \$updates\)/);
assert.match(doubanAddonService, /'warnings'\s*=>\s*\$warnings/);
assert.match(doubanAddonService, /public static function rollbackPicture/);
assert.match(doubanAddonService, /whereLike\('new_values', '%"vod_pic"%'\)/);
assert.doesNotMatch(doubanAddonService, /'vod_pic'\s*=>\s*\['vod_pic', 'pic', 'cover', 'poster'\]/);

const doubanAddonBackendController = execFileSync("tar", ["-xOf", doubanAddonArchive, "douban/backend/DoubanController.php"], { encoding: "utf8" });
assert.match(doubanAddonBackendController, /public function rollbackPic/);
assert.match(doubanAddonBackendController, /DoubanData::rollbackPicture/);
assert.match(doubanAddonBackendController, /DoubanData::auditDashboard/);
assert.match(doubanAddonBackendController, /public function startAudit/);
assert.match(doubanAddonBackendController, /DoubanData::runAuditBatch/);
assert.match(doubanAddonBackendController, /DoubanData::pauseAudit/);
assert.match(doubanAddonBackendController, /DoubanData::resumeAudit/);
assert.match(doubanAddonBackendController, /public function exportAudit\(\)[\s\S]*?if \(!\$this->isAdmin\(\)\)/);
assert.match(doubanAddonBackendController, /Request\(\)->isPost\(\)/);
assert.match(doubanAddonBackendController, /Request\(\)->isAjax\(\)/);
...
assert.match(doubanAddonView, /id="doubanOperationStatus"/);
...
assert.match(doubanAddonView, /\{if condition="!empty\(\$vo\['can_rollback_pic'\]\)"\}/);
assert.match(doubanAddonView, /douban\/rollbackPic/);
...
assert.match(doubanAddonView, /douban\/fetchVod/);
