(function () {
  ...
  var themeStorageKey = "squared_media_theme";
  var validThemes = {
  ...
  function queueSiteNotice(message, status) {
    try {
      window.sessionStorage.setItem("squared_media_site_notice", JSON.stringify({
        message: message,
        status: status
      }));
    } catch (error) {
  ...
  function removeQueuedSiteNotice() {
    try {
      window.sessionStorage.removeItem("squared_media_site_notice");
    } catch (error) {}
  }
  ...
  function showQueuedSiteNotice() {
    var raw = "";
    try {
      raw = window.sessionStorage.getItem("squared_media_site_notice");
    } catch (error) {
      return;
    }
    if (!raw) return;
  ...
  var favoritePrefix = "squared_media_favorite_";
  ...
  function bindAutoNextDocument(targetDocument) {
    if (!targetDocument || targetDocument._squaredMediaAutoNextReady) return;
    targetDocument._squaredMediaAutoNextReady = true;
  ...
  function initAutoNextPlayback() {
    ...
    if (shell && window.MutationObserver && !shell._squaredMediaAutoNextObserver) {
      shell._squaredMediaAutoNextObserver = new MutationObserver(function () {
        bindAutoNextFrames(shell);
      });
      shell._squaredMediaAutoNextObserver.observe(shell, { childList: true, subtree: true });
    }
  }
  ...
  function initDynamicVodFilters(root) {
    ...
    scope.querySelectorAll("[data-dynamic-vod-filters]").forEach(function (panel) {
      if (panel.dataset.squaredMediaFilterReady === "true") return;
      panel.dataset.squaredMediaFilterReady = "true";
  ...
  window.SquaredMedia = window.SquaredMedia || {};
  window.SquaredMedia.initSearchForms = initSearchForms;
  window.SquaredMedia.initLogoutLinks = initLogoutLinks;
  window.SquaredMedia.clearFavoriteCache = clearFavoriteCache;
  window.SquaredMedia.initFavoriteButtons = initFavoriteButtons;
  window.SquaredMedia.initPageJumpForms = initPageJumpForms;
  window.SquaredMedia.initHomeLatestTabs = initHomeLatestTabs;
  window.SquaredMedia.initAutoNextPlayback = initAutoNextPlayback;
  window.SquaredMedia.initDynamicVodFilters = initDynamicVodFilters;
  window.SquaredMedia.initThemeSwitchers = initThemeSwitchers;
  ...
  window.SquaredMedia = window.SquaredMedia || {};
  window.SquaredMedia.initHeroCarousel = initHeroCarousel;
  window.SquaredMedia.initGsapMotion = initGsapMotion;
  ...
  if (timeline) {
    try {
      var raw = localStorage.getItem("squared_media_history") || localStorage.getItem("mac_history") || "[]";
