...
assert.match(index, /mac_url_type\(\$type,\['by'\s*=>\s*'time'\],'show'\)/);
...
assert.match(preview, /url\("category", \{ sort: "latest" \}\)/);
assert.match(preview, /name: category, sort: "latest"/);
...
assert.match(phpRender, /home-shelf\-latest'/);
assert.match(phpRender, /\$tabLinks \. '<a class="home-shelf-more" href="' \. e\(path_for\('category'\)\) \. '">全部影片<\/a>'/);
