...
assert.match(index, new RegExp(`\\{maccms:vod type="${nonAdultVodTypeScope}" num="6" order="desc" by="time" cachetime="300" id="vo"\\}`));
...
assert.match(index, /class="hero-gradient-strips" data-gradient-strips aria-hidden="true"/);
assert.equal((index.match(/data-gradient-strip(?=[\s>])/g) || []).length, 5);
...
for (const [tabIndex, typeId] of [
  ...
  assert.match(index, new RegExp(`id="latest-panel-category-${tabIndex}"[\\s\\S]*?\\{maccms:vod type="${typeId}" num="6" order="desc" by="time" cachetime="300" id="vo"\\}`));
}
...
const heroGradientStripsRule = style.match(/\.hero-gradient-strips\s*\{[\s\S]*?\}/)?.[0] || "";
const heroGradientStripRule = style.match(/\.hero-gradient-strip\s*\{[\s\S]*?\}/)?.[0] || "";
...
assert.match(heroGradientStripsRule, /--strip-x: 50%/);
assert.match(heroGradientStripsRule, /--strip-y: 50%/);
assert.match(heroGradientStripsRule, /position: absolute/);
assert.match(heroGradientStripsRule, /pointer-events: none/);
assert.match(heroGradientStripsRule, /z-index: 1/);
assert.match(heroGradientStripRule, /position: absolute/);
assert.match(heroGradientStripRule, /animation: gradient-strip-drift/);
assert.match(style, /@keyframes gradient-strip-drift/);
assert.match(style, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.hero-gradient-strip\s*\{[\s\S]*animation: none/);
assert.match(appScript, /function initHeroGradientStrips\(root\)/);
assert.match(appScript, /querySelectorAll\("\[data-gradient-strips\]"\)/);
assert.match(appScript, /style\.setProperty\("--strip-x"/);
assert.match(appScript, /style\.setProperty\("--strip-y"/);
assert.match(appScript, /initHeroGradientStrips\(document\)/);
assert.match(appScript, /PingFangVideo\.initHeroGradientStrips = initHeroGradientStrips/);
...
assert.match(vodFilterOptionsService, /const CACHE_VERSION = 'v3'/);
...
assert.match(vodFilterOptionsService, /optionExists/);
assert.match(vodFilterOptionsService, /\$maccms\['app'\]\['vod_extend_' \. \$dimension\]/);
assert.match(vodFilterOptionsService, /field\('vod_id'\)->find\(\)/);
