...
assert.match(index, new RegExp(`js/gsap\\.min\\.js\\?v=3\\.15\\.0[\\s\\S]*\\{include file="public/foot" \\/\\}`));
...
assert.doesNotMatch(appScript, /initRevealMotion|IntersectionObserver|data-gsap-reveal/);
...
assert.equal(packageJson.scripts.lint, "npm run lint:template");
...
assert.match(rollbackScript, /douban\.backup\.\*/);
...
assert.match(doubanBackendController, /input\('status', 'all'\)/);
assert.match(doubanBackendController, /input\('type_id\/d', 0\)/);
assert.match(doubanBackendController, /input\('year', ''\)/);
assert.match(doubanBackendController, /DoubanData::listVideos\(\$status, \$page, \$limit, \$q, \$typeId, \$year\)/);
assert.match(doubanBackendController, /DoubanData::runPending/);
assert.match(doubanBackendController, /DoubanData::retryFailed/);
assert.match(doubanBackendController, /DoubanData::fetchVod/);
assert.match(doubanBackendController, /DoubanData::syncVod/);
assert.match(doubanBackendController, /DoubanData::rollbackPicture/);
assert.match(doubanBackendController, /public function rollbackPic/);
assert.match(doubanBackendController, /请确认回退图片/);
...
assert.match(doubanBackendController, /DoubanData::auditDashboard/);
assert.match(doubanBackendController, /public function startAudit/);
assert.match(doubanBackendController, /DoubanData::runAuditBatch/);
assert.match(doubanBackendController, /DoubanData::pauseAudit/);
assert.match(doubanBackendController, /DoubanData::resumeAudit/);
assert.match(doubanBackendController, /public function exportAudit\(\)[\s\S]*?if \(!\$this->isAdmin\(\)\)/);
assert.match(doubanBackendController, /DoubanData::auditIssueExportBatch/);
...
assert.match(doubanBackendController, /Request\(\)->isPost\(\)/);
assert.match(doubanBackendController, /Request\(\)->isAjax\(\)/);
...
assert.match(doubanDataService, /const SCAN_TABLE = 'douban_scan'/);
assert.match(doubanDataService, /const SCAN_ISSUE_TABLE = 'douban_scan_issue'/);
...
assert.match(doubanDataService, /const AUDIT_START_LOCK_KEY = 'audit_start_lock'/);
assert.match(doubanDataService, /where\('config_key', self::AUDIT_START_LOCK_KEY\)\s*->lock\(true\)/);
...
assert.match(doubanDataService, /public static function auditDashboard/);
assert.match(doubanDataService, /public static function startAudit/);
assert.match(doubanDataService, /public static function runAuditBatch/);
assert.match(doubanDataService, /public static function pauseAudit/);
assert.match(doubanDataService, /public static function resumeAudit/);
assert.match(doubanDataService, /where\('vod_id', '>', \$cursor\)/);
assert.match(doubanDataService, /where\('vod_id', '<=', \$highWaterVodId\)/);
assert.match(doubanDataService, /private static function auditVodIssues/);
assert.doesNotMatch(doubanDataService, /OFFSET\s+/i);
assert.match(doubanDataService, /public static function listVideos/);
assert.match(doubanDataService, /where\('v\.type_id', \$typeId\)/);
assert.match(doubanDataService, /where\('v\.vod_year', \$year\)/);
assert.match(doubanDataService, /where\('v\.vod_id', \(int\) \$q\)/);
assert.doesNotMatch(doubanDataService, /v\.vod_id\|v\.vod_name/);
assert.match(doubanDataService, /'total_pages'/);
assert.match(doubanDataService, /ty\.type_name/);
...
assert.match(doubanDataService, /public static function fetchVod/);
assert.match(doubanDataService, /public static function syncVod/);
assert.match(doubanDataService, /public static function rollbackPicture/);
assert.doesNotMatch(doubanDataService, /'vod_pic'\s*=>\s*\['vod_pic', 'pic', 'cover', 'poster'\]/);
assert.match(doubanDataService, /where\('action', self::ACTION_AUTO_SYNC\)/);
assert.match(doubanDataService, /whereLike\('new_values', '%"vod_pic"%'\)/);
assert.doesNotMatch(doubanDataService, /where\('action', self::ACTION_AUTO_SYNC\)[\s\S]{0,160}->limit\(50\)/);
...
assert.match(enqueueDueMethod, /INNER JOIN \{\$vodTable\} v/);
assert.doesNotMatch(enqueueDueMethod, /ORDER BY IFNULL\(m\.douban_next_sync_at/);
...
assert.match(doubanDataService, /private static function syncChangesForView/);
assert.match(doubanDataService, /'changes'\s*=>\s*self::syncChangesForView\(\$oldValues, \$updates\)/);
assert.match(doubanDataService, /'warnings'\s*=>\s*\$warnings/);
assert.match(doubanDataService, /超过本地字段上限/);
...
assert.match(doubanAddonSql, /CREATE TABLE IF NOT EXISTS `__PREFIX__douban_scan`/);
assert.match(doubanAddonSql, /CREATE TABLE IF NOT EXISTS `__PREFIX__douban_scan_issue`/);
assert.match(doubanAddonSql, /idx_scan_issue/);
assert.match(doubanAddonSql, /idx_scan_code_issue/);
...
assert.match(doubanAddonSql, /\('audit_start_lock', '0'/);
...
assert.match(doubanAddonView, /批量任务设置/);
assert.match(doubanAddonView, /不会自行启动服务器定时任务/);
assert.match(doubanAddonView, /待人工核查/);
assert.match(doubanAddonView, /无豆瓣ID/);
assert.match(doubanAddonView, /任务监控与日志/);
assert.match(doubanAddonView, /id="doubanAuditPanel"/);
assert.match(doubanAddonView, /data-douban-action="start-audit"/);
assert.match(doubanAddonView, /data-douban-action="pause-audit"/);
assert.match(doubanAddonView, /function runAuditLoop/);
assert.match(doubanAddonView, /不访问豆瓣、不修改视频表/);
assert.match(doubanAddonView, /导出 CSV/);
assert.match(doubanAddonView, /data-douban-action/);
assert.match(doubanAddonView, /id="doubanQuickSearchForm"/);
assert.match(doubanAddonView, /url\('douban\/fetchVod'\)/);
assert.match(doubanAddonView, /查找要匹配的视频/);
assert.match(doubanAddonView, /name="type_id"/);
assert.match(doubanAddonView, /name="year"/);
assert.match(doubanAddonView, /影片分类/);
assert.match(doubanAddonView, /数据年份/);
assert.match(doubanAddonView, /data-douban-action="fetch-vod"/);
assert.match(doubanAddonView, /搜索豆瓣ID/);
assert.match(doubanAddonView, /id="doubanOperationStatus"[^>]*role="status"[^>]*aria-live="polite"/);
assert.match(doubanAddonView, /data-feedback-title/);
assert.match(doubanAddonView, /setInterval\(updateElapsed, 1000\)/);
assert.match(doubanAddonView, /sessionStorage\.setItem\("doubanFeedback"/);
assert.match(doubanAddonView, /id="doubanSyncResult"[^>]*hidden/);
assert.match(doubanAddonView, /data-sync-changes/);
assert.match(doubanAddonView, /data-sync-refresh/);
assert.match(doubanAddonView, /function renderSyncResult/);
assert.match(doubanAddonView, /changeValue\.textContent/);
assert.match(doubanAddonView, /onSuccess:\s*renderSyncResult/);
assert.match(doubanAddonView, /data-douban-action="rollback-pic"/);
assert.match(doubanAddonView, /url\('douban\/rollbackPic'\)/);
assert.match(doubanAddonView, /同步不会修改现有图片/);
assert.match(doubanAddonView, /回退上次图片/);
assert.doesNotMatch(doubanAddonView, /回退本次图片/);
assert.match(doubanAddonView, /can_rollback_pic/);
assert.match(doubanAddonView, /aria-label="视频结果分页"/);
assert.match(doubanAddonView, /pagination\.prev_url/);
assert.match(doubanAddonView, /pagination\.next_url/);
assert.match(doubanAddonView, /function renderSyncWarnings/);
assert.match(doubanAddonView, /function runWorkerBatches/);
assert.match(doubanAddonView, /X-Requested-With/);
assert.match(doubanAddonView, /<details class="row-tools">/);
assert.match(doubanAddonView, /<details class="advanced-panel">/);
assert.match(doubanAddonView, /高级工具/);
assert.doesNotMatch(doubanAddonView, /<details class="advanced-panel" open/);
assert.doesNotMatch(doubanAddonView, /alert\(/);
const doubanQuickSearchIndex = doubanAddonView.indexOf('id="doubanQuickSearchForm"');
const doubanAdvancedPanelIndex = doubanAddonView.indexOf('<details class="advanced-panel">');
assert.ok(doubanQuickSearchIndex >= 0 && doubanAdvancedPanelIndex > doubanQuickSearchIndex);
...
assert.match(releaseVerifier, /public static function fetchVod/);
assert.match(releaseVerifier, /name="type_id"/);
assert.match(releaseVerifier, /name="year"/);
assert.match(releaseVerifier, /data-douban-action="fetch-vod"/);
...
assert.match(releaseVerifier, /douban\\\/fetchVod/);
...
assert.doesNotMatch(appJs, /revealBatchSize|observer\.unobserve/);
...
assert.doesNotMatch(appJs, /rank-item|home-shelf-card/);
