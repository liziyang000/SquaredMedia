...
assert.match(index, /class="liquid-lens" aria-hidden="true"/);
...
assert.match(style, /\.footer-links a\s*\{[\s\S]*?min-height: 44px/);
assert.match(style, /\.footer-links a\s*\{[\s\S]*?min-width: 44px/);
...
assert.match(appScript, /gsap\.set\(animatedSections, \{ y: 16, willChange: "transform" \}\)/);
...
assert.match(style, /@media \(max-width: 1020px\)[\s\S]*?\.header-search-wrap\s*\{[\s\S]*?display: block[\s\S]*?grid-column: 2/);
...
assert.match(preview, /class="liquid-lens" aria-hidden="true"/);
...
assert.match(preview, /class="shelf-title"><small>NEW THIS WEEK<\/small>/);
...
assert.match(phpRender, /class="liquid-lens" aria-hidden="true"/);
...
assert.match(phpRender, /\$latestShelf = '<section class="wrap home-shelf home-shelf-latest"[\s\S]*<span class="shelf-title"><small>NEW THIS WEEK<\/small><h2>最新上线<\/h2><\/span>' \. \$tabLinks \. '<a class="home-shelf-more" href="' \. e\(path_for\('category'\)\) \. '">全部影片<\/a>/);
...
assert.match(appJs, /function initLiquidLens/);
assert.match(appJs, /gsap\.quickTo/);
...
assert.match(homePreview, /class="liquid-lens" aria-hidden="true"/);
