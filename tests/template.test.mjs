...
const bannerArtRule = style.match(/\.banner-art\s*\{[\s\S]*?\}/)?.[0] || "";
const bannerPosterRule = style.match(/\.banner-poster\s*\{[\s\S]*?\}/)?.[0] || "";
...
assert.match(heroGridRule, /grid-template-columns: minmax\(0, 1\.14fr\) minmax\(300px, 340px\)/);
assert.match(heroGridRule, /align-items: stretch/);
assert.match(heroCarouselRule, /min-height: 0/);
assert.match(heroCarouselRule, /background: linear-gradient\(145deg, rgba\(24, 27, 34, 0\.96\), rgba\(8, 10, 14, 0\.98\)\)/);
assert.match(bannerTrackRule, /position: relative/);
assert.match(bannerTrackRule, /min-height: 0/);
...
assert.match(heroRankRule, /gap: 10px/);
assert.match(heroRankRule, /padding: 20px/);
assert.match(heroRankBeforeRule, /background: linear-gradient\(90deg, var\(--accent-2\), var\(--accent\)\)/);
assert.match(heroRankHeadRule, /position: relative/);
assert.match(heroRankHeadRule, /padding-bottom: 6px/);
assert.match(rankItemRule, /grid-template-columns: 34px minmax\(0, 1fr\)/);
assert.match(rankItemRule, /align-items: start/);
assert.match(rankItemRule, /min-height: 0/);
assert.match(rankItemRule, /border: 1px solid rgba\(255, 255, 255, 0\.08\)/);
assert.match(rankItemRule, /border-radius: var\(--radius\)/);
assert.match(rankItemRule, /background: rgba\(255, 255, 255, 0\.035\)/);
assert.match(rankItemRule, /padding: 11px 12px/);
assert.match(rankFirstItemRule, /background: linear-gradient\(135deg, rgba\(255, 90, 61, 0\.18\), rgba\(38, 212, 175, 0\.08\)\)/);
assert.match(rankFirstItemRule, /padding: 15px 14px/);
assert.match(rankFirstIndexRule, /width: 42px/);
assert.match(rankFirstIndexRule, /height: 42px/);
assert.match(rankIndexRule, /width: 34px/);
...
assert.match(heroSlideRule, /grid-template-columns: minmax\(0, 0\.9fr\) minmax\(220px, 304px\)/);
assert.match(heroSlideRule, /grid-template-areas: "content art"/);
assert.match(heroSlideRule, /position: absolute/);
assert.match(heroSlideRule, /min-height: clamp\(430px, 31vw, 520px\)/);
assert.match(heroSlideRule, /padding: 42px 48px 78px/);
assert.match(heroSlideRule, /gap: 38px/);
...
assert.match(bannerPosterRule, /position: relative/);
assert.match(bannerArtRule, /grid-area: art/);
assert.match(bannerArtRule, /place-items: center/);
assert.match(style, /\.banner-art::before\s*\{[\s\S]*background: radial-gradient/);
assert.match(style, /\.banner-art::before\s*\{[\s\S]*border-radius: 999px/);
assert.match(bannerPosterRule, /justify-self: end/);
assert.match(bannerPosterRule, /width: min\(100%, 276px\)/);
assert.match(bannerPosterRule, /transform: translateY\(-6px\)/);
assert.match(bannerCopyRule, /max-width: 560px/);
assert.match(bannerCopyRule, /min-width: 0/);
assert.match(bannerTitleRule, /overflow: visible/);
assert.match(bannerTitleRule, /line-height: 1\.16/);
...
assert.match(bannerControlsRule, /right: 48px/);
assert.match(bannerControlsRule, /bottom: 24px/);
assert.match(bannerControlsRule, /border-radius: 999px/);
assert.match(bannerControlsRule, /backdrop-filter: blur\(12px\)/);
assert.match(bannerControlsRule, /padding: 8px 10px/);
...
assert.match(bannerDotRule, /min-width: 14px/);
...
assert.match(style, /@media \(max-width: 760px\)[\s\S]*\.hero-rank \.rank-item:first-of-type\s*\{[\s\S]*grid-template-columns: 36px minmax\(0, 1fr\)/);
assert.match(style, /@media \(max-width: 760px\)[\s\S]*\.hero-rank \.rank-item:first-of-type \.rank-index\s*\{[\s\S]*width: 36px/);
assert.match(style, /@media \(max-width: 760px\)[\s\S]*\.rank-item\s*\{[\s\S]*grid-template-columns: 30px minmax\(0, 1fr\)/);
assert.match(style, /@media \(max-width: 760px\)[\s\S]*\.rank-index\s*\{[\s\S]*width: 30px/);
...
assert.match(style, /\.mobile-drawer-login\s*\{[\s\S]*border-color: rgba\(40, 199, 167, 0\.42\)/);
...
assert.match(style, /@media \(max-width: 760px\)[\s\S]*\.banner-controls\s*\{[\s\S]*bottom: 16px/);
...
assert.match(style, /@media \(max-width: 520px\)[\s\S]*\.hero-slide\s*\{[\s\S]*padding: 18px 14px 78px/);
assert.match(style, /@media \(max-width: 520px\)[\s\S]*\.banner-content\s*\{[\s\S]*min-height: 318px/);
